...
class panoHDR(object):
    ...
    def test(self):
        ...
        for sample_file in sample_files:
            ...
            image_paths = [os.path.join(self.args.out_dir, '{0}_out.exr'.format(os.path.basename(sample_file).split('.')[0])),
                           ...
                           os.path.join(self.args.out_dir, '{0}.exr'.format(os.path.basename(sample_file).split('.')[0]))]
    ...
    def load(self, checkpoint_dir):
        print(" [*] Reading checkpoint from ", checkpoint_dir, " ...")

        ckpt = tf.train.get_checkpoint_state(checkpoint_dir)
        if ckpt and ckpt.model_checkpoint_path:
            ckpt_name = os.path.basename(ckpt.model_checkpoint_path)
            print(" [*] Found checkpoint: ", ckpt_name)
